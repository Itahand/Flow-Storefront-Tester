...
pub contract NPMContract: NonFungibleToken {
    
    // -----------------------------------------------------------------------
    // NPM contract Event definitions
    // -----------------------------------------------------------------------
    ...
    // NPMContract state level variables 
    ...
    */
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
        ...
        init(name: String, description: String, media: String, author: Address,data: {String: String}?) {
            NPMContract.totalSupply = NPMContract.totalSupply + 1
            self.id = NPMContract.totalSupply
            self.name = name
    ...
    /** NPMContractCollectionPublic
    *   A public interface extending the standard NFT Collection with type information specific
    *   to NPMContract NFTs.
    */
    pub resource interface NPMContractCollectionPublic {
        pub fun deposit(token: @NonFungibleToken.NFT)
        pub fun getIDs(): [UInt64]
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
       // pub fun borrowEntireNFT(id: UInt64): &NPMContract.NFT
        pub fun borrowNFTNPMContractContract(id: UInt64): &NPMContract.NFT? {
    ...
    */
    pub resource Collection: NPMContractCollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            // Cast the deposited token as a NPMContract NFT to make sure
            // it is the correct type
            let token <- token as! @NPMContract.NFT
            // Get the token's ID
            let id = token.id
        ...
        // borrowNFTNPMContractContract returns a borrowed reference to a NPMContractNFT
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver} {
            ...
            let exampleNFT = nft as! &NPMContract.NFT
    ...
    pub resource NFTAdminResource: NFTAdminResourcePublic {
        ...
        pub fun mintToken(name: String, description: String, media: String, data: {String: String}?, recipientAddress: Address) {
            ...
            let recipientCollection = receiptAccount
                .getCapability(NPMContract.CollectionPublicPath)
        ...
        pub fun setMintingFee(mintngfee: UFix64) {
            ...
            NPMContract.tokenMintingFee = mintngfee
        }
    ...
    init() {
        ...
        self.CollectionStoragePath = /storage/NPMContractNFTCollection
        self.NFTAdminResourceStoragePath = /storage/NFTAdminResourceStoragePath
        self.CollectionPublicPath = /public/NPMContractNFTCollection
        self.NFTAdminResourcePublicPath = /public/NFTAdminResourcePublicPath
        self.CollectionPrivatePath = /private/NPMContractNFTCollection

        self.account.save(<- create NFTAdminResource(), to: self.NFTAdminResourceStoragePath)
        self.account.link<&{NPMContract.NFTAdminResourcePublic}>(self.NFTAdminResourcePublicPath, target: self.NFTAdminResourceStoragePath)
